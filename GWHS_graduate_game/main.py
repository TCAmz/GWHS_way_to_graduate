...
class QuoteBird(pygame.sprite.Sprite):
    def __init__(self):
        ...
        if quote_first_time:
            self.random_quote = "Good morning GWHS, \nit's another day where \nEagles fly high!"
            self.font = pygame.font.Font(None, 32)
            quote_first_time = False
        self.font_surf = self.font.render(self.random_quote, True, (0,0,0))
        self.font_rect = self.font_surf.get_rect(center = ((screen_width+ 150)+self.image.width/1.5, 125))
    def update(self):
        self.rect.x -= 7
        self.font_rect.x -= 7
...
def spawn_a_row_stuff(stuff_speed, highest_y):
    ...
    for i in range(0,4):
        new_y += 75
        original_x+= 400
        new_stuff = Stuff(False, stuff_speed, highest_y, ["document", "pencil", "clipboard"])
        new_stuff.rect.y = new_y 
...
def in_game_scene():
    ...
    if quote_time_passed >= 7:
        quote_group.add(QuoteBird())
        quote_time_passed -= 7
...
def level_4():
    ...
    stuff_speed = 25
    rate = random.randint(0, 100)
    if barrier_time_passed >= 10:
        barrier_group.add(Barrier())
        barrier_time_passed -= 10
...
def reset_value():
    ...
    clear_group(stuff_group)
    clear_group(quote_group)
    clear_group(barrier_group)
...
while running:
    ...
    if game_scene == 0:
        ...
        if level <=3 or score <50:
            result_display()
        else:
            screen.blit(the_end_scene, (0,0))
            screen.blit(end_button, exit_button_rect)
            screen.blit(play_again_button, play_again_button_rect)
